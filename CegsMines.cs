...
public partial class CegsMines : Cegs
{
    ...
    protected virtual void OpenTF_VS1()
    {
        ...
        var step = ProcessStep.Start($"Open and evacuate TF and {vacuumSystem.Name}.");
    ...
    {
        var step = ProcessStep.Start($"Fill {InletPort.Name} to {pAmbient.Pressure:0} Torr He");
    ...
    {
        var step = ProcessStep.Start($"Admit {TfPressureTarget:0} Torr O2 into {InletPort.Name}");
    ...
    protected void Bakeout()
    {
        ...
        SetParameter(nameof(TfPressureTarget), 50);
        AdmitO2toTF();
        SetParameter(nameof(IpSetpoint), 500);
        TurnOnIpSampleFurnace();
        WaitIpRiseToSetpoint();
        StartFlowThroughToWaste();
